...
public class BrokenLineTrendView extends View {
     ...
     * 虚线的颜色
     */
    private int mDottedLineColor  = getResources().getColor(R.color.EFEFEF);
    /**
     * 文字的颜色
     */
     ...
     * 折线的粗细
     */
    private float mBrokenLineWith = DensityUtil.dip2px(getContext(),1.3f);
    /**
     * 字体大小
     */
     ...
     * padding 值
     */
    private int mLeftPadding = DensityUtil.dip2px(getContext(), 10);
    ...
    private void drawDottedLine(Canvas canvas) {
        ...
        if (count > 0) {
            canvas.save();
            //mDottedPaint.setPathEffect(new DashPathEffect(new float[]{20, 10}, 4)); //虚线
        ...
        mDottedPaint.setStrokeWidth(2f);
        canvas.drawLine(56, 780, 0, 200, mDottedPaint);

       /* for (int i = 1; i * YScale <= YLength; i++) {//画横刻度
            canvas.drawLine(0, YPoint - i * YScale, XLength, YPoint - i* YScale, canvas);
    ...
    private void drawYLineText(Canvas canvas) {
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                String text = String.valueOf(mYLineDataList.get(count - 1 - i)).substring(0,String.valueOf(mYLineDataList.get(count - 1 - i)).length()-2);
        ...
        if (dataSize > 0) {
            ...
            for (int j = 0; j < dataSize; j++) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    canvas.drawCircle(mScorePointsList.get(j).get(i).x, mScorePointsList.get(j).get(i).y, DensityUtil.dip2px(getContext(), 1.8f), mBrokenPointPaint);
                    if (mSelectIndex == i) {
